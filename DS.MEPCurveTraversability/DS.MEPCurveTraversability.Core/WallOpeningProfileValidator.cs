...
namespace DS.MEPCurveTraversability.Core
{
    /// <summary>
    ...
    public class WallOpeningProfileValidator<TIntersectItem> : IValidator<(Wall, TIntersectItem)>
    {
        ...
        public bool IsValid((Wall, TIntersectItem) value)
        {
            ...
            if (!wall.TryGetBasis(_activeDoc, _links, out var wallBasis))
            { throw new Exception("Failed to get wall basis"); }

            var wall1Faces = GeometryElementsUtils.GetFaces(wall, _activeDoc, _links);
            var w1PlanarFaces = wall1Faces.OfType<PlanarFace>().ToList();
            var wall1YFaces = w1PlanarFaces.FindAll(FaceFilter.YNormal(wall, _activeDoc, _links));
            ...
            { throw new Exception("Failed to create wall rectangle"); }
            _transactionFactory?.CreateAsync(() => { wallRectangle.Show(_activeDoc); }, "ShowWallEdges");

            (List<Face> wallFaces, Dictionary<ElementId, List<Face>> insertsFacesCollection) = wall.GetFaces(_activeDoc, _links, _geomOptions, true);
            ...
            return IsValidOpening(wallRectangle, externalJointRectangels, insertsRectangles, null, openingRectangle);
        }
            ...
            PlanarFace wallYFace2, Document doc, double jointsOffset, Options geomOptions)
        {
            ...
            foreach (var id in jointsIds)
            {
                ...
                {
                    var jointFaces = GeometryElementsUtils.GetFaces(jWall, _activeDoc, _links).OfType<PlanarFace>().ToList();
                    if (!jWall.TryGetBasis(_activeDoc, _links, out var bBasis))
                    { throw new Exception("Failed to get join basis"); }
                    ...
                    if (!wallYFace1.TryProject(rect, true, out var projRect)) { throw new Exception("Failed to project"); }

                    if (!projRect.TryExtend(jointsOffset, out var exRectangle))
                    { throw new Exception("Failed to extend rectangle"); }
                                                                    ...
                                                                    double wallOffset)
        {
            ...
            var (freeEdges, joinElementsEdges) = GeometryElementsUtils.GetSplitedEdges(wall, doc, _links, true);
            ...
            var mainOrigin = mainPlane.ToRhinoPlane().ClosestPoint(origin.ToPoint3d()).ToXYZ();
